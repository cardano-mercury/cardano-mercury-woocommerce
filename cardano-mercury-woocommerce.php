...
use Mercury\TaskManager;
use Mercury\Transaction;

require_once 'vendor/autoload.php';
require_once __DIR__ . '/lib/action-scheduler/action-scheduler.php';
require_once __DIR__ . '/lib/checkout-block/woocommerce-gateway-cardano-mercury.php';
...
/**
 * Filters to add Cardano payment currency, ADA currency symbol, and the Mercury
 * gateway
 */
		...
		class WC_Gateway_Cardano_Mercury extends WC_Payment_Gateway {
			...
			public function __construct() {
				...
				$this->icon                 = apply_filters("cardano_mercury_icon",
					plugin_dir_url(__FILE__) . '/mercury-woocommerce-logo.png');
				$this->has_fields           = true;
				$this->method_title         = _x("Cardano Mercury", "payment method title", "woocommerce");
			...
			public function needs_setup(): bool {
				...
				if (empty($options['blockfrostAPIKey'])) {
					$this->log->error("Blockfrost API key is not defined!", setup_log);

					return true;
				}

				if (empty($options['walletAddress'])) {
					$this->log->error("Wallet address is not defined!", setup_log);
						...
						<div class="native-asset-selection">
							<p class="form-row form-row-wide native-asset-selection">
								<label for="cardano_currency">
									<?= __("Choose the Cardano Native Asset you will use to pay",
										mercury_text_domain); ?>
								</label>
							...
							<p class="form-row form-row-wide" id="minUTxO">
								Note: if using a Native Asset other than ADA (&#8371;) for payment, your total amount
								due reflects a minimum amount of ADA (minUTxO) required to accompany the native asset of
								your choosing.
							</p>
			...
			public function process_payment($order_id): ?array {
				...
				try {
					...
					$this->log->error("Could not fetch ADA Price Details?\r\n{$e->getMessage()}", processing_log);
				...
				if (!$ADAPrice) {
					...
					$this->log->error("ADA Price is 0?!", processing_log);
				...
				if (!$ADATotal) {
					...
					$this->log->error("ADA Price is 0?! {$ADATotal} {$usdTotal}", processing_log);
				...
				$this->log->debug("What is the Cardano currency? {$cardano_currency}", processing_log);
			...
			protected function get_usd(string $fromCurr, int $interval = 300): float {
				...
				if (!$rate) {
					...
					if (!$rate) {
						$this->log->error("Count not find the rate for currency conversion? " . wc_print_r($result,
								true));
...
//            return round($total * $rate, 2);
			...
			private function getPaymentDetails($order_id) {
				...
				return $wpdb->get_row($wpdb->prepare("SELECT * FROM {$order_value_table} WHERE order_id = %d",
					$order_id));
			}
...
function setup_mercury_cron() {
	...
	if (class_exists('ActionScheduler') && ActionScheduler::is_initialized('as_has_scheduled_action')) {
		TaskManager::maybeSchedule('mercury_cron_hook', $mercury_settings['cronFrequency']);
		TaskManager::maybeSchedule('mercury_sync_assets', $mercury_settings['taptoolCronFrequency']);
		// TODO: Update this to an option to be set
...
function setup_mercury_tables() {
    ...
    CREATE TABLE {$order_value_table} IF NOT EXISTS (
